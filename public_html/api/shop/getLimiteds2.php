...
if(isset($_GET['sort'])){
  ...
  if(isset($_GET['page'])){
    ...
    $limited = "limited";
    if($sort!='all'){
      $getItems = $conn->prepare("SELECT * FROM `items_brz` WHERE `type`=? AND `status`=? ORDER BY `id` DESC") or die($conn->error);
      $getItems->bind_param("ss", $sort, $limited) or die($conn->error);
    }else{
      $getItems = $conn->prepare("SELECT * FROM `items_brz` WHERE `status`=? ORDER BY `id` DESC") or die($conn->error);
      $getItems->bind_param("s", $limited) or die($conn->error);
    }
  }
