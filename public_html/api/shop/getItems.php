...
if(isset($_GET['sort']) && isset($_GET['page'])){
  ...
  $getItems = $conn->prepare("SELECT * FROM `items_brz` WHERE `type`=? ORDER BY `id` DESC LIMIT 12 OFFSET ?") or die($conn->error);
  $getItems->bind_param("ss", $sort, $offset) or die($conn->error);
